...
/**
 * ============================================================
 *  B8X — Armazenamento de leads do Quiz do SETOR FUNERÁRIO
 * ============================================================
 *
 *  O QUE FAZ:
 *   • Cria o menu "Leads Funerário B8X" no painel do WordPress.
 *   • Recebe os dados do quiz (b8x-funeraria-quiz.html) e grava
 ...
 *     (Auto Insert) e ative.
 *
 *  Observação: este snippet é independente do da elétrica — pode ter
 *  os dois ativos ao mesmo tempo, cada um com seu próprio menu.
 * ============================================================
 */
 ...
 * ============================================================ */
if ( ! defined( 'B8X_FUN_NOTIFY_EMAIL' ) ) {
	define( 'B8X_FUN_NOTIFY_EMAIL', 'blaneckpaloma@gmail.com, leandro@b8x.com.br' );
}

/* 1) Registra o tipo de conteúdo dos leads do funerário */
add_action( 'init', function () {
	register_post_type( 'b8x_lead_fun', array(
		'labels' => array(
			'name'          => 'Leads Funerário B8X',
			'singular_name' => 'Lead Funerário',
			'menu_name'     => 'Leads Funerário B8X',
...
add_action( 'wp_ajax_b8x_save_lead_fun',        'b8x_save_lead_fun' );
add_action( 'wp_ajax_nopriv_b8x_save_lead_fun', 'b8x_save_lead_fun' );

function b8x_save_lead_fun() {
	...
	if ( B8X_FUN_NOTIFY_EMAIL ) {
		$assunto = 'Novo lead Funerário B8X' . ( $nome ? ' — ' . $nome : '' );
		$corpo   = "Alguém preencheu o quiz do setor funerário.\n\n";
		...
		$destinatarios = array_filter( array_map( 'trim', explode( ',', B8X_FUN_NOTIFY_EMAIL ) ) );
...
add_filter( 'manage_b8x_lead_fun_posts_columns', function ( $cols ) {
	return array(
		'cb'              => $cols['cb'],
...
add_action( 'manage_b8x_lead_fun_posts_custom_column', function ( $col, $post_id ) {
	if ( 'b8x_whatsapp' === $col ) {
...
add_action( 'add_meta_boxes', function () {
	add_meta_box( 'b8x_lead_fun_details', 'Dados do Lead', function ( $post ) {
		$fields = array(
			'b8x_nome'        => 'Nome',
			'b8x_whatsapp'    => 'WhatsApp',
